        ...
        <div className="mb-4">
          <div className="relative">
            <input
              ...
              className="w-full p-2 pl-10 rounded-lg bg-white/10 text-white placeholder-white/50 border border-white/20 focus:outline-none focus:ring-2 focus:ring-white/30"
            />
          ...
          <div className="relative">
            <div className="flex overflow-x-auto pb-2 space-x-2 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
              {chapters.map((chapter: HadithChapter) => (
                ...
                >
                  <span>{chapter.titleBn || chapter.title}</span>
        ...
        {isLoading ? (
          ...
          <div className="divide-y divide-gray-100">
            {filteredHadiths.map((hadith: HadithResponse) => (
              <div key={`hadith-${hadith.id}-${hadith.bookSlug}-${hadith.hadithNumber}`} className="p-4 hover:bg-gray-50">
                <div className="flex items-center justify-between mb-2">
                  <h3 className="font-bold text-gray-800">হাদিস নং {hadith.hadithNumber}</h3>
                ...
                <p className="text-gray-600 mb-3 leading-relaxed whitespace-pre-line">
                  {hadith.bodyBn || hadith.body}
                </p>
                
                <div className="text-sm text-gray-500 flex items-center justify-between">
                  <div>Reference: {selectedBook} {hadith.hadithNumber}</div>
                </div>
              </div>
            ))}
